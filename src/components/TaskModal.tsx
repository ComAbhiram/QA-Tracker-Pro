...
import { X, Save, Calendar, User, Briefcase, Activity, Layers, Plus, CheckCircle2, AlertCircle, Clock, Copy } from 'lucide-react';
...
export default function TaskModal({ isOpen, onClose, task, defaultAssigneeName, onSave, onDelete, isDuplicate }: TaskModalProps) {
    ...
    const [initialDuplicateData, setInitialDuplicateData] = useState<{ formData: any, assignees: any } | null>(null);
    ...
    useEffect(() => {
        if (isOpen && task) {
            ...
            const finalAssignees = initialAssignees.length === 0 ? [{ ...defaultAssignee, name: defaultAssigneeName || null }] : initialAssignees;
            setAssignees(finalAssignees);

            if (isDuplicate) {
                setInitialDuplicateData({ formData: data, assignees: finalAssignees });
            } else {
                setInitialDuplicateData(null);
            }

        } else if (isOpen && !task) {
            setFormData(initialState);
            setAssignees([{ ...defaultAssignee, name: defaultAssigneeName || null }]);
            setInitialDuplicateData(null);
        }
        if (isOpen) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Duplication Check
        if (isDuplicate && initialDuplicateData) {
            const formDataChanged = JSON.stringify(formData) !== JSON.stringify(initialDuplicateData.formData);
            const assigneesChanged = JSON.stringify(assignees) !== JSON.stringify(initialDuplicateData.assignees);
            
            if (!formDataChanged && !assigneesChanged) {
                toast.error("Same task already exists. Please discard or edit fields to continue.");
                return;
            }
        }
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90dvh] overflow-y-auto custom-scrollbar border border-slate-100 dark:border-slate-800 transition-colors duration-300">
                ...
                <form onSubmit={handleSubmit} className="p-4 pb-10 md:p-6 space-y-8">
                    ...
                    <div className="space-y-3">
                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2"><Layers size={16} className="text-indigo-500" /> Task</label>
                        <Combobox options={subPhases} value={formData.subPhase || ''} onChange={(val) => setFormData(prev => ({ ...prev, subPhase: val ? String(val) : '' }))} placeholder="Select task..." allowCustomValue={true} isLoading={loadingSubPhases} />
                    </div>
