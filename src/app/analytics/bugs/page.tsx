...
import { AlertCircle, Bug, Code2, Zap, Search, ArrowUpDown, X, ChevronDown, ChevronRight, Clock, CheckCircle, Download } from 'lucide-react';
...
export default function BugsReport() {
    ...
    const exportCSV = () => {
        const headers = ['Project Name', 'Task Name', 'Status', 'HTML Bugs', 'Functional Bugs', 'Total Bugs', 'Comments'];
        const rows: any[] = [];
        ...
        processedData.forEach(project => {
            project.projectTasks.forEach(task => {
                rows.push([
                    ...
                    `"${(task.subPhase || task.projectType || 'General Task').replace(/"/g, '""')}"`,
                    `"${task.status}"`,
                    task.htmlBugs || 0,
                    task.functionalBugs || 0,
                    task.bugCount || 0,
        ...
        link.download = `Bugs_Report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
        ...
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="flex flex-col sm:flex-row gap-3 bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-colors w-full sm:w-auto">
                        {/* Search */}
                    ...
                    <button
                        onClick={exportCSV}
                        className="glint-effect flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all font-bold text-sm shadow-lg shadow-indigo-100 dark:shadow-none h-[62px] sm:h-auto w-full sm:w-auto justify-center"
                    >
                        <Download size={18} />
                        Export CSV
                    </button>
