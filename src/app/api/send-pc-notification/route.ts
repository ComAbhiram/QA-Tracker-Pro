...
interface PCNotificationRequest {
...
export async function POST(request: NextRequest) {
    if (!BREVO_API_KEY) {
        console.error('[Brevo API] API key missing');
        return NextResponse.json({ error: 'Brevo API key not configured' }, { status: 503 });
    }

    try {
        const body: PCNotificationRequest = await request.json();
        const {
            type,
            pcEmail,
            pcName,
        ...
        console.error('[Brevo API] Unexpected error:', error);
