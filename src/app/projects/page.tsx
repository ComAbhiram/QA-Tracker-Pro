            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`btn flex items-center gap-2 text-sm px-4 py-2 rounded-lg transition-all ${activeTab === 'import' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                    >
                        <Globe size={16} /> <span className="hidden sm:inline">Import from Hubstaff</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('create')}
                        className={`btn flex items-center gap-2 text-sm px-4 py-2 rounded-lg transition-all ${activeTab === 'create' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                    >
                        <Plus size={16} /> <span className="hidden sm:inline">Create Manual</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('list')}
                        className={`btn flex items-center gap-2 text-sm px-4 py-2 rounded-lg transition-all ${activeTab === 'list' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                    >
