...
export default function Tracker() {
    ...
    const handleAddTask = () => {
        setEditingTask(null);
        setDefaultAssigneeName(null);
        setIsTaskModalOpen(true);
    };

    const handleAddTaskForAssignee = (name: string) => {
        setEditingTask(null);
        setDefaultAssigneeName(name);
        setIsTaskModalOpen(true);
    };
    ...
    const saveTask = async (taskData: Partial<Task> | Partial<Task>[]) => {
        ...
        if (Array.isArray(taskData)) {
            ...
            if (editingTask && !isDuplicateTask) {
    ...
    const exportCSV = () => {
        const headers = ['Project Name', 'Type', 'Priority', 'Task', 'Status', 'Start Date', 'End Date', 'Actual End', 'Assignees', 'Bug Count', 'HTML Bugs', 'Functional Bugs', 'Comments', 'Current Updates'];
        const csvContent = [
            headers.join(','),
            ...tasks.map(t => [
                `"${t.projectName}"`,
    ...
    return (
        <div className="max-w-[1920px] mx-auto pb-20 px-4 sm:px-6 lg:px-8"> {/* Added horizontal padding to prevent overflow */}
                ...
                <table className="w-full text-xs text-slate-800 dark:text-slate-200 border-collapse table-fixed">
                    ...
                    <thead className="bg-slate-55/40 dark:bg-slate-900/60 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider border-b border-slate-200/20 dark:border-slate-750/25">
                        <tr>
                            ...
                            <ResizableHeader label="Task" widthKey="subPhase" width={columnWidths.subPhase} onResizeStart={startResizing} />
                            ...
                            {sortedAssignees.map(assignee => (
                                <DraggableTableWrapper key={assignee} id={assignee}>
                                    <AssigneeTaskTable
                                        ...
                                        onDuplicateTask={isPCMode ? () => { } : handleDuplicateTask}
                                        onFieldUpdate={isPCMode ? async () => { } : handleFieldUpdate}
                                        onLeaveUpdate={fetchLeaves}
                                        onAddTask={handleAddTaskForAssignee}
                                    />
            ...
            <TaskModal
                ...
                onSave={saveTask}
                onDelete={editingTask && !isDuplicateTask ? () => handleDeleteTask(editingTask.id) : undefined}
            />
